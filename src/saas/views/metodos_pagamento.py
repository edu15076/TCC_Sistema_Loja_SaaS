...
class MetodosPagamentoCRDView(
    ...
    def get_context_data(self, **kwargs):
        ...
        cartao_padrao = Cartao.cartoes.filter(
            cliente_contratante=self.user, padrao=True
        ).first()
        ...
        context['cartoes_count'] = Cartao.cartoes.filter(
    ...
    def get_template_names(self):
        ...
        if request is not None:
            if (
                self.forms_class['cartao_padrao'].submit_name() in request.POST
                or self.user.cartoes.count() == 1
            ):
    ...
    def get_form(self, form_class=None, form_name=None, request=None):
        if form_class is not None or form_name is not None:
            ...
            return super().get_form(
                form_class=self.forms_class['cartao'], request=request
            )
