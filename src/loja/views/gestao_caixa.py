...
class GestaoCaixaCRUDListView(ABCGestaoCaixaCRUDListView, View):
    # TODO: Revisar essa view, pouco especializada
    ...
    def post(self, request: HttpRequest, *args, **kwargs) -> HttpResponse:
        ...
        if acao == "adicionar":
            ...
            if Caixa.objects.filter(numero_identificacao=numero_identificacao,
                ...
                request.session[
                    "error_message"] = "Esse número de identificação já está cadastrado. Por favor, escolha outro."
                return redirect(
                    reverse('gestao_caixas', kwargs={'loja_scope': loja_scope}))
            ...
            if int(numero_identificacao) < 1 or int(numero_identificacao) > 99999999:
                request.session[
                    "error_message"] = "O número de identificação deve estar entre 00000001 e 99999999."
                return redirect(
                    reverse('gestao_caixas', kwargs={'loja_scope': loja_scope}))
            ...
            try:
                ...
                request.session[
                    "error_message"] = "Já existe um caixa cadastrado com esse número de identificação."
                return redirect(
                    reverse('gestao_caixas', kwargs={'loja_scope': loja_scope}))
            ...
            try:
                ...
                request.session[
                    "error_message"] = f"O caixa com este número de identificação {numero_identificacao} é inválido."
                return redirect(
                    reverse('gestao_caixas', kwargs={'loja_scope': loja_scope}))
            ...
            if numero_identificacao:
                caixa_pesquisa = get_object_or_404(Caixa,
                                                   numero_identificacao=numero_identificacao,
                                                   loja__scope=loja_scope)
                caixeiros_encontrados = caixeiros_encontrados.filter(
                    caixa=caixa_pesquisa)
            ...
            if caixeiros_encontrados.exists():
                ...
                for trabalho in caixeiros_encontrados:
                    for timeslice in trabalho.trabalho_por_dia.timeslices.all():
                        dia_da_semana = self.get_dia_da_semana(
                            trabalho.trabalho_por_dia.dia_da_semana)
            ...
            try:
                ...
                request.session[
                    "error_message"] = "Ocorreu um erro ao remover o período."
            ...
            try:
                if acao == "abrir":
                    ...
                    for dia in dias_trabalho:
                        ...
                        horario_inicio_time = datetime.strptime(horario_inicio,
                                                                '%H:%M').time()
                        horario_fim_time = datetime.strptime(horario_fim,
                                                             '%H:%M').time()

                        for trabalho in trabalhos_existentes:
                            for timeslice in trabalho.trabalho_por_dia.timeslices.all():
                                if (
                                        horario_inicio_time < timeslice.end and horario_fim_time > timeslice.start):
                                    request.session[
                                        "error_message"] = f"O caixeiro {caixeiro.nome} já está associado a um horário nesse dia."
                                    return redirect(reverse('gestao_caixas', kwargs={
                                        'loja_scope': loja_scope}))

                    time_slice = TimeSlice.objects.create(start=horario_inicio_time,
                                                          end=horario_fim_time)
