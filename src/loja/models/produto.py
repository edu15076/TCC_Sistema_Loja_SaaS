...
class Produto(ValidateModelMixin, models.Model):
    ...
    codigo_de_barras = models.CharField(_('Código de barras'), max_length=128, blank=True)
    ...
    def qtd_em_estoque(self):
        return sum([p.qtd_em_estoque for p in self.lotes.produtos_por_lote.filter(produto=self)])
    ...
    def promocao_por_data(self, data: datetime) -> 'Promocao' | None:
        ...
        promocao = self.promocoes.filter(
            data_inicio__lte=data
        ).annotate(
            tempo_total=F('periodo__tempo_total')
        ).filter(
            tempo_total__gte=(data - F('data_inicio'))
        ).first()
...
class ProdutoPorLote(models.Model):
    ...
    qtd_em_estoque = models.IntegerField(_('Quantidade'), validators=[MinValueValidator(0, _('Quantidade não pode ser negativo.'))])
    produto = models.ForeignKey(Produto, verbose_name=_('Produto'), on_delete=models.RESTRICT, editable=False, related_name='lotes')
